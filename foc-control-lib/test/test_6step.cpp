...
TEST(SixStep, SectorLookup) {
    ...
    CHECK_EQUAL(0, six_step_get_sector(DEG_TO_RAD(30.0f)));
    CHECK_EQUAL(1, six_step_get_sector(DEG_TO_RAD(90.0f)));
    CHECK_EQUAL(2, six_step_get_sector(DEG_TO_RAD(150.0f)));
    CHECK_EQUAL(3, six_step_get_sector(DEG_TO_RAD(210.0f)));
    CHECK_EQUAL(4, six_step_get_sector(DEG_TO_RAD(270.0f)));
    CHECK_EQUAL(5, six_step_get_sector(DEG_TO_RAD(330.0f)));
}

TEST(SixStep, SixStepSectorDutyCycleSanity) {
    for (uint8_t i = 0; i < SIX_STEP_NUM_SECTORS; i++) {
        const float test_angle = DEG_TO_RAD(60.0f * i) + DEG_TO_RAD(30.0f);
